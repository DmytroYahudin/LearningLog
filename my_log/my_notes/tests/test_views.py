...
class TopicsViewTest(TestCase):
    ...
    def test_topics_view_context(self):
        ...
        self.assertTrue(
            list(response.context_data["themes"])
            == list(Topic.objects.order_by("date_added"))
        )
...
class ThemeViewTest(TestCase):
        ...
        for topic in range(number_of_topics):
            if topic % 2 == 0:
                ...
                entry = Entry.objects.create(
                    topic=theme, text=f"test entry {topic}", owner=user1
                )
                entry.save()
            else:
                ...
                theme.save()
                entry = Entry.objects.create(
                    topic=theme, text=f"test entry {topic}", owner=user2
                )
                entry.save()
    ...
    def test_topic_view_context(self):
        ...
        self.assertIsInstance(response.context_data, dict)
        self.assertEqual(
            str(response.context_data["topic"]), str(Topic.objects.get(pk=1))
        )
    ...
    def test_for_wrong_topic_id(self):
        ...
        response = self.client.get(
            reverse("my_notes:topic", kwargs={"topic_id": last_id + 1})
        )
...
class NewTopicViewTest(TestCase):
    ...
    def test_post_method(self):
        ...
        response = self.client.post(
            reverse("my_notes:new_topic"), {"text": new_topic, "owner": "testuser1"}
        )
...
class NewEntryViewTest(TestCase):
    ...
    def test_logged_user_gets_correct_template(self):
        self.client.login(username="testuser1", password="123")
        response = self.client.get(
            reverse("my_notes:new_entry", kwargs={"topic_id": 1})
        )
        self.assertEqual(str(response.context["user"]), "testuser1")
        self.assertEqual(response.status_code, 200)
    ...
    def test_for_getting_404_code(self):
        self.client.login(username="testuser1", password="123")
        response = self.client.get(
            reverse("my_notes:new_entry", kwargs={"topic_id": 2})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_post_method(self):
        ...
        response = self.client.post(
            reverse("my_notes:new_entry", kwargs={"topic_id": 1}),
            {"text": new_entry, "owner": "testuser1"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("my_notes:topic", kwargs={"topic_id": 1})
        )
        ...
        self.assertEqual(
            updated_number_of_entries_in_topic, initial_number_of_entries_in_topic + 1
        )
...
class EditEntryViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse("my_notes:edit_entry", kwargs={"entry_id": 1})
        )
    ...
    def test_for_getting_404_code(self):
        self.client.login(username="testuser1", password="123")
        response = self.client.get(
            reverse("my_notes:edit_entry", kwargs={"entry_id": 2})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_post_method(self):
        ...
        self.assertEqual(
            updated_number_of_entries_in_topic, initial_number_of_entries_in_topic
        )
...
class DeleteEntryViewTest(TestCase):
    ...
    def test_delete_post_request(self):
        self.client.login(username="testuser1", password="123")
        response = self.client.post(
            reverse("my_notes:delete_entry", kwargs={"pk": 1}), follow=True
        )
        self.assertRedirects(
            response, reverse("my_notes:topic", kwargs={"topic_id": 1}), status_code=302
        )
